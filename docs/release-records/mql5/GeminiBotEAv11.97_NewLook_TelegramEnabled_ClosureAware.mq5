...
//=== Remote API Settings ===
const string   API_URL              = "https://signal.fizuxc0der.uk/config"; // Cloudflare HTTPS MasterServer config endpoint
const int      CONFIG_TIMEOUT_MS    = 5000;                            // Allow DNS/TLS/Cloudflare Tunnel latency
...
input string   Gemini_Event_Ingest_Key    = "GeminiBotEA-4445";                      // Dedicated portal key; never use MASTER_SERVER_SYNC_KEY
...
input bool     Enable_Telegram_Signal     = true;                    // Enabled: send validated setup metadata to the owner-configured Telegram channel
...
const string   GEMINI_TELEGRAM_SIGNAL_URL = "https://fizuxea-jxctlods.manus.space/api/telegram/signals/gemini";
...
input double           Min_TP_Profit_USD  = 100.0;       // Default Minimum Basket Profit (USD) (Overridden by API)

//=== Virtual Basket Trailing Stop Engine ===
input bool     Use_Trailing_Stop = true;      // Enable Virtual Basket Trailing Stop
input double   Trail_Start_Pips  = 100.0;      // Basket profit required to activate lock (Pips)
input double   Trail_Step_Pips   = 70.0;       // Trailing distance behind current price (Pips)
...
input datetime CustomStartDate       = D'2026.08.25 00:00';  // ROI Start Date
...
double runtime_Min_TP_Profit_USD = 100.0;
...
// Telegram lifecycle reporting state. These variables only report milestones; they never place, modify, or close orders.
string   last_telegram_signal_event_id = "";
...
ResetLastError();
int res = WebRequest("GET", url_with_params, NULL, NULL,
                     CONFIG_TIMEOUT_MS, post, 0, result, headers);
...
if(res == 200) {
    ...
    if(TimeCurrent() - last_err_print > 10) {
        if(request_error == 4060) {
            ...
            Print("❌ CONFIG FAIL url=", url_with_params,
                  " http=", res,
                  " mt5=", request_error,
                  " response=", StringSubstr(response_body, 0, 240));
        }
...
string EaClockTime24(datetime value)
{
MqlDateTime parts;
TimeToStruct(value, parts);
...
string EaClockDateDdmmyyyy(datetime value)
{
MqlDateTime parts;
TimeToStruct(value, parts);
...
string fibo_targets = "";
if(setup_tp1 > 0.0) fibo_targets += ",\"fiboTp1\":\"" + DoubleToString(setup_tp1, _Digits) + "\"";
...
Print("Telegram setup signal event=", event_id, " TP1=", DoubleToString(setup_tp1, _Digits), " TP2=", DoubleToString(setup_tp2, _Digits), " TP3=", DoubleToString(setup_tp3, _Digits), " SL(-1.0)=", DoubleToString(setup_sl_neg100, _Digits), " date=", EaClockDateDdmmyyyy(gmt8_time), " time=", EaClockTime24(gmt8_time), " GMT+8 HTTP=", status, " MT5Error=", GetLastError(), " response=", StringSubstr(response_text, 0, 160));
if((status == 201 || status == 200) && StringFind(response_text, "\"delivered\":true") >= 0)
{
    last_telegram_signal_event_id = event_id;
...
{
    if(!Enable_Telegram_Signal || last_telegram_signal_event_id == "" || last_telegram_signal_direction == 0) return;
...
string TelegramStatePrefix()
{
    return StringFormat("GeminiTG_%I64d_%s_%s_", AccountInfoInteger(ACCOUNT_LOGIN), StringSubstr(_Symbol, 0, 24), EnumToString(_Period));
}

string TelegramStateKey(string suffix)
{
    return TelegramStatePrefix() + suffix;
}

void ClearTelegramLifecycleState()
{
    GlobalVariableDel(TelegramStateKey("time"));
    GlobalVariableDel(TelegramStateKey("direction"));
    GlobalVariableDel(TelegramStateKey("tp1"));
    GlobalVariableDel(TelegramStateKey("tp2"));
    GlobalVariableDel(TelegramStateKey("tp3"));
...
bool HasManagedExposure()
{
    for(int i = 0; i < PositionsTotal(); i++)
        if(posInfo.SelectByIndex(i) && IsManagedPosition()) return true;
...
// Returns only whether managed market positions remain. Pending orders do not make a “Closed all” position result true.
...
void PersistTelegramLifecycleState(datetime setup_time)
{
    ...
    GlobalVariableSet(TelegramStateKey("tp2"), locked_fibo_tp2);
    GlobalVariableSet(TelegramStateKey("tp3"), locked_fibo_tp3);
...
{
    if(!HasManagedExposure()) { ClearTelegramLifecycleState(); return; }
    if(!GlobalVariableCheck(TelegramStateKey("time"))) { ClearTelegramLifecycleState(); return; }
    datetime setup_time = (datetime)GlobalVariableGet(TelegramStateKey("time"));
    if(setup_time <= 0 || (long)TimeCurrent() - (long)setup_time > 172800) { ClearTelegramLifecycleState(); return; }
    last_telegram_signal_direction = (int)GlobalVariableGet(TelegramStateKey("direction"));
    if(last_telegram_signal_direction != 1 && last_telegram_signal_direction != -1) { ClearTelegramLifecycleState(); return; }
    locked_fibo_tp1 = GlobalVariableGet(TelegramStateKey("tp1"));
    locked_fibo_tp2 = GlobalVariableGet(TelegramStateKey("tp2"));
    locked_fibo_tp3 = GlobalVariableGet(TelegramStateKey("tp3"));
...
double point = SymbolInfoDouble(_Symbol, SYMBOL_POINT);
double current_tp_1272 = 0, current_tp1 = 0, current_tp2 = 0, current_tp3 = 0, current_sl_neg100 = 0;
double diff = 0;
double anchor_a = 0;
double anchor_b = 0;
...
if(is_buy_breakout)
{
    ...
    current_sl_neg100 = anchor_a - (diff * 1.0);
...
{ 
    ...
    locked_fibo_tp2 = current_tp2; 
        ...
        locked_fibo_sl_neg100 = current_sl_neg100;
...
if(Use_Macro_Filter) {
    ...
    if(current_macro != "NEUTRAL" && grand_macro != "NEUTRAL" && current_macro != grand_macro) {
        ...
        if(macro_pre_tp > 0.0) {
            // Apply the pullback cap only when basket direction matches
            // the current macro direction. An opposite-direction manual
            // basket retains its own directional Safe TP.
            int macro_direction = (current_macro == "BULLISH") ? 1 : -1;
            if(macro_direction == 1 && locked_trend == 1 && active_basket_tp > macro_pre_tp) {
                active_basket_tp = macro_pre_tp;
                active_basket_tp_label = "H4 PULLBACK CAP (TP1)";
            }
            else if(macro_direction == -1 && locked_trend == -1 && active_basket_tp < macro_pre_tp) {
                active_basket_tp = macro_pre_tp;
                active_basket_tp_label = "H4 PULLBACK CAP (TP1)";
            }
        }
...
void CreateText(string name, int x, int y, string text, int size, color clr, bool bold = false, ENUM_ANCHOR_POINT anchor = ANCHOR_LEFT_UPPER)
{
...
//+------------------------------------------------------------------+
//| MODERN HUD DASHBOARD GENERATOR                                   |
//+------------------------------------------------------------------+
void DrawDashboard()
{
DeleteDashboard();
...
// UI Colors
color bgMain = C'12,15,20';
color bgPanel = C'22,26,34';
color bgHead = C'18,22,30';
color borderPanel = C'40,46,60';
color borderAccent = clrDeepSkyBlue;

// 1. Backgrounds
CreatePanel("GBUI_Main", DX, DY, W, H, bgMain, borderPanel);
CreatePanel("GBUI_Head", DX, DY, W, 72, bgHead, borderAccent);

// 2. Header Content
CreateText("GBUI_Title", DX+15, DY+10, "💎 GEMINI QUANT BOT V11.97", 14, clrWhite, true);
CreateText("GBUI_License", DX+15, DY+51, "👑 VIP LICENSE: VERIFYING...", 9, clrGold, true);
CreateText("GBUI_Time", DX+W-15, DY+30, "Sunday 00:00:00 (GMT+8)", 10, clrSilver, true, ANCHOR_RIGHT_UPPER);

// 3. Panel 1: Account Health
int P1_Y = DY + 82;
CreatePanel("GBUI_P1", DX+10, P1_Y, 220, 110, bgPanel, borderPanel);
CreateText("GBUI_H1", DX+20, P1_Y+8, "🏦 ACCOUNT HEALTH", 9, clrDodgerBlue, true);

CreateText("GBUI_L_Bal", DX+20, P1_Y+30, "Balance:", 9, clrSilver);
CreateText("GBUI_V_Bal", DX+220, P1_Y+30, "0.00", 9, clrWhite, true, ANCHOR_RIGHT_UPPER);

CreateText("GBUI_L_DD", DX+20, P1_Y+50, "Drawdown:", 9, clrSilver);
CreateText("GBUI_V_DD", DX+220, P1_Y+50, "0.00%", 9, clrOrange, true, ANCHOR_RIGHT_UPPER);

CreateText("GBUI_L_DPL", DX+20, P1_Y+70, "Daily P/L:", 9, clrSilver);
CreateText("GBUI_V_DPL", DX+220, P1_Y+70, "0.00", 9, clrWhite, true, ANCHOR_RIGHT_UPPER);

CreateText("GBUI_L_ROI", DX+20, P1_Y+90, "Total ROI:", 9, clrSilver);
CreateText("GBUI_V_ROI", DX+220, P1_Y+90, "0.00%", 9, clrWhite, true, ANCHOR_RIGHT_UPPER);

// 4. Panel 2: Active Engine
CreatePanel("GBUI_P2", DX+240, P1_Y, 220, 110, bgPanel, borderPanel);
CreateText("GBUI_H2", DX+250, P1_Y+8, "📊 ACTIVE ENGINE", 9, clrMediumPurple, true);

CreateText("GBUI_L_Pos", DX+250, P1_Y+30, "Positions:", 9, clrSilver);
CreateText("GBUI_V_Pos", DX+450, P1_Y+30, "0", 9, clrWhite, true, ANCHOR_RIGHT_UPPER);

CreateText("GBUI_L_Vol", DX+250, P1_Y+50, "Total Volume:", 9, clrSilver);
CreateText("GBUI_V_Vol", DX+450, P1_Y+50, "0.00", 9, clrWhite, true, ANCHOR_RIGHT_UPPER);

CreateText("GBUI_L_FPL", DX+250, P1_Y+70, "Floating P/L:", 9, clrSilver);
CreateText("GBUI_V_FPL", DX+450, P1_Y+70, "0.00", 9, clrWhite, true, ANCHOR_RIGHT_UPPER);

CreateText("GBUI_L_Pips", DX+250, P1_Y+90, "Net Pips:", 9, clrSilver);
CreateText("GBUI_V_Pips", DX+450, P1_Y+90, "0.0", 9, clrWhite, true, ANCHOR_RIGHT_UPPER);

// 5. Panel 3: Quant Intelligence
int P3_Y = P1_Y + 120;
CreatePanel("GBUI_P3", DX+10, P3_Y, 220, 90, bgPanel, borderPanel);
CreateText("GBUI_H3", DX+20, P3_Y+8, "🧠 QUANT INTELLIGENCE", 9, clrMediumSpringGreen, true);

CreateText("GBUI_L_Reg", DX+20, P3_Y+30, "Mkt Regime:", 9, clrSilver);
CreateText("GBUI_V_Reg", DX+220, P3_Y+30, "SCANNING", 9, clrDarkGray, true, ANCHOR_RIGHT_UPPER);

CreateText("GBUI_L_Mac", DX+20, P3_Y+50, "Macro Trend:", 9, clrSilver);
CreateText("GBUI_V_Mac", DX+220, P3_Y+50, "SCANNING...", 9, clrWhite, true, ANCHOR_RIGHT_UPPER);

CreateText("GBUI_L_ML", DX+20, P3_Y+70, "ML Signal:", 9, clrSilver);
CreateText("GBUI_V_ML", DX+220, P3_Y+70, "INITIALIZING", 9, clrGold, true, ANCHOR_RIGHT_UPPER);

// 6. Panel 4: Profit Targets
CreatePanel("GBUI_P4", DX+240, P3_Y, 220, 90, bgPanel, borderPanel);
CreateText("GBUI_H4", DX+250, P3_Y+8, "🎯 PROFIT TARGETS", 9, clrOrange, true);

CreateText("GBUI_L_Targ", DX+250, P3_Y+30, "Target (USD):", 9, clrSilver);
CreateText("GBUI_V_Targ", DX+450, P3_Y+30, "0.00", 9, clrDeepSkyBlue, true, ANCHOR_RIGHT_UPPER);

CreateText("GBUI_L_STP", DX+250, P3_Y+50, "Safe TP:", 9, clrSilver);
CreateText("GBUI_V_STP", DX+450, P3_Y+50, "WAITING", 9, clrPlum, true, ANCHOR_RIGHT_UPPER);

CreateText("GBUI_L_ETP", DX+250, P3_Y+70, "Est. Profit:", 9, clrSilver);
CreateText("GBUI_V_ETP", DX+450, P3_Y+70, "0.00", 9, clrMediumAquamarine, true, ANCHOR_RIGHT_UPPER);

// 7. Panel 5: Sequence Tracker
int P5_Y = P3_Y + 100;
CreatePanel("GBUI_P5", DX+10, P5_Y, 450, 60, bgPanel, borderPanel);
CreateText("GBUI_H5", DX+20, P5_Y+8, "🚀 PRICE ACTION SEQUENCE TRACKER", 9, clrGold, true);
CreateText("GBUI_V_Seq", DX+20, P5_Y+30, "STANDBY", 11, clrGold, true);

// 8. Action Buttons
int Btn_Y = P5_Y + 70;
ObjectCreate(0, "GBUI_BtnClose", OBJ_BUTTON, 0, 0, 0);
ObjectSetInteger(0, "GBUI_BtnClose", OBJPROP_XDISTANCE, DX + 10);
ObjectSetInteger(0, "GBUI_BtnClose", OBJPROP_YDISTANCE, Btn_Y);
ObjectSetInteger(0, "GBUI_BtnClose", OBJPROP_XSIZE, 450); 
ObjectSetInteger(0, "GBUI_BtnClose", OBJPROP_YSIZE, 35);
...
// 9. Info/Alert Text
CreateText("GBUI_Info", DX+(W/2), Btn_Y + 45, "Auto-Adapting TP / Break-Even Protection Active", 8, clrDarkGray, false, ANCHOR_UPPER);
...
ObjectSetString(0, "GBUI_V_Bal", OBJPROP_TEXT, DoubleToString(bal, 2) + " " + acc_curr);
ObjectSetString(0, "GBUI_V_DD", OBJPROP_TEXT, DoubleToString(dd, 2) + "%");
if(dd >= Hard_Stop_DD && Use_Hard_Stop) ObjectSetInteger(0, "GBUI_V_DD", OBJPROP_COLOR, clrCrimson);
else if(dd > 0.0) ObjectSetInteger(0, "GBUI_V_DD", OBJPROP_COLOR, clrOrange);
else ObjectSetInteger(0, "GBUI_V_DD", OBJPROP_COLOR, clrWhite);

double daily_pl = GetDailyProfit();
ObjectSetString(0, "GBUI_V_DPL", OBJPROP_TEXT, DoubleToString(daily_pl, 2) + " " + acc_curr);
if(daily_pl > 0) ObjectSetInteger(0, "GBUI_V_DPL", OBJPROP_COLOR, clrLime);
else if(daily_pl < 0) ObjectSetInteger(0, "GBUI_V_DPL", OBJPROP_COLOR, clrCrimson);
else ObjectSetInteger(0, "GBUI_V_DPL", OBJPROP_COLOR, clrWhite);
...
string growth_str = StringFormat("%s%.2f (%.2f%%)", (total_profit > 0 ? "+" : ""), total_profit, growth_pct);

ObjectSetString(0, "GBUI_V_ROI", OBJPROP_TEXT, growth_str);
if(total_profit > 0) ObjectSetInteger(0, "GBUI_V_ROI", OBJPROP_COLOR, clrLime);
else if(total_profit < 0) ObjectSetInteger(0, "GBUI_V_ROI", OBJPROP_COLOR, clrCrimson);
else ObjectSetInteger(0, "GBUI_V_ROI", OBJPROP_COLOR, clrWhite);

// Active Engine Updates
int total_trades = 0;
double t_profit = 0.0;
double t_volume = 0.0;
double t_value = 0.0;
...
ObjectSetString(0, "GBUI_V_Pos", OBJPROP_TEXT, IntegerToString(total_trades));
ObjectSetString(0, "GBUI_V_Vol", OBJPROP_TEXT, DoubleToString(t_volume, 2));

ObjectSetString(0, "GBUI_V_FPL", OBJPROP_TEXT, DoubleToString(t_profit, 2) + " " + acc_curr);
if(t_profit > 0) ObjectSetInteger(0, "GBUI_V_FPL", OBJPROP_COLOR, clrLime);
else if(t_profit < 0) ObjectSetInteger(0, "GBUI_V_FPL", OBJPROP_COLOR, clrCrimson);
else ObjectSetInteger(0, "GBUI_V_FPL", OBJPROP_COLOR, clrWhite);
...
ObjectSetString(0, "GBUI_V_Pips", OBJPROP_TEXT, DoubleToString(net_pips, 1));
if(net_pips > 0) ObjectSetInteger(0, "GBUI_V_Pips", OBJPROP_COLOR, clrLime);
else if(net_pips < 0) ObjectSetInteger(0, "GBUI_V_Pips", OBJPROP_COLOR, clrCrimson);
else ObjectSetInteger(0, "GBUI_V_Pips", OBJPROP_COLOR, clrWhite);

// Quant Intelligence Updates
ObjectSetString(0, "GBUI_V_Reg", OBJPROP_TEXT, runtime_Regime);
if(runtime_Regime == "QUIET") ObjectSetInteger(0, "GBUI_V_Reg", OBJPROP_COLOR, clrMediumSpringGreen);
else if(runtime_Regime == "NORMAL") ObjectSetInteger(0, "GBUI_V_Reg", OBJPROP_COLOR, clrDeepSkyBlue);
else if(runtime_Regime == "EXTREME VOLATILITY") ObjectSetInteger(0, "GBUI_V_Reg", OBJPROP_COLOR, clrOrangeRed);
else ObjectSetInteger(0, "GBUI_V_Reg", OBJPROP_COLOR, clrDarkGray);
...
ObjectSetString(0, "GBUI_V_Mac", OBJPROP_TEXT, macro_disp);
if(macro_state == "BULLISH") ObjectSetInteger(0, "GBUI_V_Mac", OBJPROP_COLOR, clrLime);
else if(macro_state == "BEARISH") ObjectSetInteger(0, "GBUI_V_Mac", OBJPROP_COLOR, clrCrimson);
else ObjectSetInteger(0, "GBUI_V_Mac", OBJPROP_COLOR, clrDarkGray);

ObjectSetString(0, "GBUI_V_ML", OBJPROP_TEXT, runtime_MLStatus);
if(runtime_MLDirection == "CHOPPY") ObjectSetInteger(0, "GBUI_V_ML", OBJPROP_COLOR, clrDarkGray);
else if(runtime_WinProb >= 65.0) ObjectSetInteger(0, "GBUI_V_ML", OBJPROP_COLOR, clrLime);
else if(runtime_WinProb <= 55.0) ObjectSetInteger(0, "GBUI_V_ML", OBJPROP_COLOR, clrCrimson);
else ObjectSetInteger(0, "GBUI_V_ML", OBJPROP_COLOR, clrGold);

// Profit Targets Updates
double live_target = runtime_Min_TP_Profit_USD * target_multi;
ObjectSetString(0, "GBUI_V_Targ", OBJPROP_TEXT, DoubleToString(live_target, 2) + " " + acc_curr);

if(Use_Fibo_TP && active_basket_tp > 0) {
    ObjectSetString(0, "GBUI_V_STP", OBJPROP_TEXT, DoubleToString(active_basket_tp, _Digits) + " (" + active_basket_tp_label + ")");
    ...
    ObjectSetString(0, "GBUI_V_ETP", OBJPROP_TEXT, DoubleToString(est_profit, 2) + " " + acc_curr);
    
    if(est_profit > 0) ObjectSetInteger(0, "GBUI_V_ETP", OBJPROP_COLOR, clrLime);
    else if(est_profit < 0) ObjectSetInteger(0, "GBUI_V_ETP", OBJPROP_COLOR, clrCrimson);
    else ObjectSetInteger(0, "GBUI_V_ETP", OBJPROP_COLOR, clrMediumAquamarine);

} else if(!Use_Fibo_TP) {
    ObjectSetString(0, "GBUI_V_STP", OBJPROP_TEXT, "DISABLED");
    ObjectSetString(0, "GBUI_V_ETP", OBJPROP_TEXT, "0.00 " + acc_curr);
    ObjectSetInteger(0, "GBUI_V_ETP", OBJPROP_COLOR, clrDarkGray);
} else {
    ObjectSetString(0, "GBUI_V_STP", OBJPROP_TEXT, "WAITING");
    ObjectSetString(0, "GBUI_V_ETP", OBJPROP_TEXT, "0.00 " + acc_curr);
    ObjectSetInteger(0, "GBUI_V_ETP", OBJPROP_COLOR, clrDarkGray);
}

// Sequence Tracker Info Updates
ObjectSetString(0, "GBUI_V_Seq", OBJPROP_TEXT, reentry_status_msg);
if(StringFind(reentry_status_msg, "STANDBY") >= 0) {
    ObjectSetInteger(0, "GBUI_V_Seq", OBJPROP_COLOR, clrDarkGray); 
}
    ...
    ObjectSetInteger(0, "GBUI_V_Seq", OBJPROP_COLOR, clrLime); 
}
    ...
    ObjectSetInteger(0, "GBUI_V_Seq", OBJPROP_COLOR, clrCrimson); 
}
    ...
    ObjectSetInteger(0, "GBUI_V_Seq", OBJPROP_COLOR, clrDeepSkyBlue);
}
else if(StringFind(reentry_status_msg, "BASKET ACTIVE") >= 0) {
    ObjectSetInteger(0, "GBUI_V_Seq", OBJPROP_COLOR, clrGold);
}
else {
    ObjectSetInteger(0, "GBUI_V_Seq", OBJPROP_COLOR, clrOrange); 
}

// Virtual Trail Alert / Baseline Info
