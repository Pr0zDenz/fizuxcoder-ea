  ...
  return <div className="min-h-screen bg-[#17201f] px-5 py-10 text-[#f4f0e8] lg:px-10 lg:py-14" data-testid="marketing-studio">
    <main className="mx-auto max-w-[1240px]">
      ...
      <section className="mt-6 rounded-[1.5rem] border border-[#6de0d8]/35 bg-[linear-gradient(130deg,rgba(14,175,167,.16),rgba(12,21,20,.62))] p-5 shadow-[4px_4px_0_rgba(14,175,167,.14)] lg:p-6" data-testid="telegram-growth-funnel">
        ...
        <div className="mt-5 grid gap-3 border-t border-[#6de0d8]/20 pt-5 lg:grid-cols-[1fr_auto_auto]"><label className="block"><span className="font-mono text-[9px] font-bold uppercase tracking-[.12em] text-[#6de0d8]">One-time agreed template approval</span><input value={templateApprovalPhrase} onChange={event => setTemplateApprovalPhrase(event.target.value)} placeholder="Type APPROVE PRIVATE THREADS TEMPLATE SET" className="mt-2 w-full rounded-xl border border-white/15 bg-black/20 px-3 py-2.5 text-sm text-[#f4f0e8] outline-none transition placeholder:text-[#8fa8a0] focus:border-[#6de0d8]" /></label><button type="button" className="button-outline self-end !border-[#6de0d8]/50 !text-[#f4f0e8]" onClick={() => verifyInvite.mutate()} disabled={verifyInvite.isPending}>{verifyInvite.isPending ? <Loader2 size={15} className="animate-spin" /> : <ShieldCheck size={15} />}Validate secure invite</button><button type="button" className="button-primary self-end !bg-[#0eafa7]" onClick={() => approveTemplateSet.mutate({ confirmationPhrase: templateApprovalPhrase as "APPROVE PRIVATE THREADS TEMPLATE SET" })} disabled={approveTemplateSet.isPending || templateApprovalPhrase !== "APPROVE PRIVATE THREADS TEMPLATE SET"}>{approveTemplateSet.isPending ? <Loader2 size={15} className="animate-spin" /> : <Check size={15} />}Approve template set</button></div><div className="mt-3"><button type="button" className="button-outline !border-[#e5a631]/55 !text-[#f4f0e8]" onClick={() => prepareGrowth.mutate()} disabled={prepareGrowth.isPending}>{prepareGrowth.isPending ? <Loader2 size={15} className="animate-spin" /> : <Sparkles size={15} />}Create invite drafts</button></div>
      </section>
