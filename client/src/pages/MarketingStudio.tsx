...
export default function MarketingStudio() {
  ...
  const approve = trpc.marketing.approve.useMutation({
    ...
    onSuccess: async result => { setMessage(`Approved and published to Threads${result.hasImage ? " with one image" : " as text-only"}. External post ID: ${result.externalPostId}.`); try { await refresh(); } catch { setMessage(`Published to Threads. External post ID: ${result.externalPostId}. Refresh the page to reload the queue.`); } },
  ...
  const retryPublish = trpc.marketing.retryPublish.useMutation({
    ...
    onSuccess: async result => { setMessage(`Threads publication retry succeeded${result.hasImage ? " with one image" : " as text-only"}. External post ID: ${result.externalPostId}.`); try { await refresh(); } catch { setMessage(`Published to Threads after retry. External post ID: ${result.externalPostId}. Refresh the page to reload the queue.`); } },
