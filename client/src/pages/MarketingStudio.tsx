...
export default function MarketingStudio() {
  ...
  const approve = trpc.marketing.approve.useMutation({
    onSuccess: async result => { setMessage(`Approved and published to Threads${result.hasImage ? " with one image" : " as text-only"}. External post ID: ${result.externalPostId}.`); await refresh(); },
    onError: error => setMessage(error.message),
  });
  const retryPublish = trpc.marketing.retryPublish.useMutation({
    onSuccess: async result => { setMessage(`Threads publication retry succeeded${result.hasImage ? " with one image" : " as text-only"}. External post ID: ${result.externalPostId}.`); await refresh(); },
    onError: error => setMessage(error.message),
  });
