...
const TELEGRAM_GROWTH_TEMPLATES = [
  {
    key: "telegram-growth-process",
    title: "Watch the process, not a promise",
    copy: "Nak tengok how a structured MT5 setup is shared? Join the Gemini Bot EA Signal channel to follow setup context, TP/SL progress and risk reminders. Bukan call untuk copy blindly—study the process, then decide ikut risk plan sendiri. Join here: {{TELEGRAM_JOIN_LINK}}\n\n#GeminiBotEA #MT5 #TradingMalaysia #RiskFirst",
  },
  {
    key: "telegram-growth-screenshot",
    title: "Signal screenshot with full context",
    copy: "Signal screenshot boleh bagi quick overview, but context tetap penting—symbol, direction, entry, TP, SL, market condition and your own risk limits. Nak follow the full signal flow with clear updates? Join: {{TELEGRAM_JOIN_LINK}}\n\nHistorical or live evidence is not a forecast or guaranteed result. #GeminiBotEA #XAUUSD #RiskManagement",
  },
  {
    key: "telegram-growth-demo",
    title: "Start with observation, not urgency",
    copy: "Kalau interested dengan EA signal workflow, observe dulu. Review a few setups, understand how TP/SL updates work, then test your own plan on demo. Jangan rush sebab one screenshot. Join the channel: {{TELEGRAM_JOIN_LINK}}\n\nTrading involves risk. #MT5 #DemoFirst #TradingMalaysia",
  },
] as const;
...
export default function MarketingStudio() {
  ...
  const applyGeminiRevision = trpc.marketing.applyGeminiBotRevision.useMutation({
    onSuccess: async result => { setMessage(`Evergreen Gemini Bot EA queue ready: ${result.created} created, ${result.revised} revised, ${result.current} already current, ${result.archived} prior drafts archived, ${result.skipped} protected from change. All new items remain drafts; no Threads post was sent.`); await refresh(); },
    onError: error => setMessage(error.message),
  });
  ...
  const copyTelegramTemplate = async (key: string, copy: string) => {
    try {
      await navigator.clipboard.writeText(copy);
      setCopiedTemplate(key);
      window.setTimeout(() => setCopiedTemplate(current => current === key ? null : current), 1800);
    } catch {
      setMessage("Copy failed in this browser. Select the Telegram template text manually before using it.");
    }
  };
  ...
  return <div className="min-h-screen bg-[#17201f] px-5 py-10 text-[#f4f0e8] lg:px-10 lg:py-14" data-testid="marketing-studio">
    <main className="mx-auto max-w-[1240px]">
      ...
      <section className="mt-6 rounded-[1.25rem] border border-[#7db88e]/40 bg-[#7db88e]/10 p-5"><p className="font-mono text-[10px] font-bold uppercase tracking-[.14em] text-[#d3f1d8]">Signal evidence intake · draft only</p><p className="mt-2 max-w-4xl text-sm leading-6 text-[#d7e1dc]">Authenticated Gemini Bot EA setup and take-profit screenshots can arrive from the VPS at <code className="rounded bg-black/20 px-1.5 py-0.5 text-xs">POST /api/threads/gemini-event</code>. Each event creates one image-backed **owner-review draft** with a portal link; it does not publish to Threads, post back to Telegram, or alter the EA signal channel. Duplicate event IDs are ignored.</p></section>

      <section className="mt-6 rounded-[1.25rem] border border-[#6de0d8]/40 bg-[#0eafa7]/10 p-5"><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div><p className="font-mono text-[10px] font-bold uppercase tracking-[.14em] text-[#6de0d8]">Telegram community growth kit · owner copy only</p><p className="mt-2 max-w-3xl text-sm leading-6 text-[#d7e1dc]">These copy templates are intentionally separate from the EA signal engine. Replace <code className="rounded bg-black/20 px-1.5 py-0.5 text-xs">{"{{TELEGRAM_JOIN_LINK}}"}</code> with your chosen public channel link or controlled private invite link. Copying a template does not publish it anywhere.</p></div><div className="rounded-xl border border-[#6de0d8]/25 bg-black/10 px-3 py-2 text-xs leading-5 text-[#c7d1cb]">Screenshot rule: use the full signal context; blur account IDs, balances, and personal details; retain the event date/time; avoid cropped “profit-only” proof.</div></div><div className="mt-5 grid gap-4 lg:grid-cols-3">{TELEGRAM_GROWTH_TEMPLATES.map(template => <article key={template.key} className="rounded-2xl border border-white/10 bg-black/10 p-4"><h2 className="font-display text-2xl tracking-[-.04em] text-[#f4f0e8]">{template.title}</h2><p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-[#d7e1dc]">{template.copy}</p><button type="button" onClick={() => void copyTelegramTemplate(template.key, template.copy)} className="button-outline mt-4 !border-[#6de0d8]/50 !text-[#f4f0e8]">{copiedTemplate === template.key ? <Check size={15} /> : <Clipboard size={15} />}{copiedTemplate === template.key ? "Copied" : "Copy template"}</button></article>)}</div></section>
      ...
      <section className="mt-8 space-y-5">
        ...
        {!studio.isLoading && !studio.data?.length && <div className="rounded-[1.5rem] border border-dashed border-white/20 p-10 text-center"><p className="font-display text-3xl">No pilot drafts yet.</p><p className="mx-auto mt-3 max-w-md text-sm leading-6 text-[#c7d1cb]">Use the evergreen refresh action or send an authenticated VPS event to add approval-gated Gemini Bot EA drafts. This creates private draft records only.</p></div>}
      </section>
    </main>
  </div>;
}
