...
function isEcosystemInfographicDraft(item: { complianceFlags: string | null }) {
...
export default function MarketingStudio() {
  ...
  const prepareGrowth = trpc.marketing.prepareTelegramGrowthDrafts.useMutation({
    onSuccess: async result => { setMessage(`Private Telegram-growth templates ready: ${result.created} created, ${result.existing} already retained. Approve this agreed template set once before it may enter the schedule.`); await refresh(); },
    onError: error => setMessage(error.message),
  });
  const prepareEcosystemGrowth = trpc.marketing.prepareEcosystemGrowthDrafts.useMutation({
    onSuccess: async result => { setMessage(`Gemini + 3 Serangkai ecosystem infographic drafts ready: ${result.created} created, ${result.existing} already retained. They remain outside the active schedule until you review and select each item.`); await refresh(); },
    onError: error => setMessage(error.message),
  });
  const approveTemplateSet = trpc.marketing.approveInitialTelegramGrowthTemplates.useMutation({
    onSuccess: async result => { setMessage(`Agreed private Telegram-growth template set approved: ${result.approved} template(s) placed in the eligible queue. VPS screenshots still require individual review.`); setTemplateApprovalPhrase(""); await refresh(); },
    onError: error => setMessage(error.message),
  });
  ...
  return <div className="min-h-screen bg-[#17201f] px-5 py-10 text-[#f4f0e8] lg:px-10 lg:py-14" data-testid="marketing-studio">
    <main className="mx-auto max-w-[1240px]">
      ...
      <section className="mt-6 rounded-[1.5rem] border border-[#6de0d8]/35 bg-[linear-gradient(130deg,rgba(14,175,167,.16),rgba(12,21,20,.62))] p-5 shadow-[4px_4px_0_rgba(14,175,167,.14)] lg:p-6" data-testid="telegram-growth-funnel">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-3"><FunnelStep number="01" title="Secure link" detail="Store and validate a revocable private t.me invite only in administrator configuration." active={Boolean(automation.data?.inviteLinkConfigured)} /><FunnelStep number="02" title="Approve agreed templates" detail="One owner-confirmed template-set approval may queue the agreed copy; screenshots still require individual review." active={(automation.data?.queuedCount ?? 0) > 0} /><FunnelStep number="03" title="Release deliberately" detail="Schedule remains paused by default; it may publish one eligible queue item per approved run." active={!automationPaused} /></div>
        <div className="mt-5 grid gap-3 border-t border-[#6de0d8]/20 pt-5 lg:grid-cols-[1fr_auto_auto]"><label className="block"><span className="font-mono text-[9px] font-bold uppercase tracking-[.12em] text-[#6de0d8]">One-time agreed template approval</span><input value={templateApprovalPhrase} onChange={event => setTemplateApprovalPhrase(event.target.value)} placeholder="Type APPROVE PRIVATE THREADS TEMPLATE SET" className="mt-2 w-full rounded-xl border border-white/15 bg-black/20 px-3 py-2.5 text-sm text-[#f4f0e8] outline-none transition placeholder:text-[#8fa8a0] focus:border-[#6de0d8]" /></label><button type="button" className="button-outline self-end !border-[#6de0d8]/50 !text-[#f4f0e8]" onClick={() => verifyInvite.mutate()} disabled={verifyInvite.isPending}>{verifyInvite.isPending ? <Loader2 size={15} className="animate-spin" /> : <ShieldCheck size={15} />}Validate secure invite</button><button type="button" className="button-primary self-end !bg-[#0eafa7]" onClick={() => approveTemplateSet.mutate({ confirmationPhrase: templateApprovalPhrase as "APPROVE PRIVATE THREADS TEMPLATE SET" })} disabled={approveTemplateSet.isPending || templateApprovalPhrase !== "APPROVE PRIVATE THREADS TEMPLATE SET"}>{approveTemplateSet.isPending ? <Loader2 size={15} className="animate-spin" /> : <Check size={15} />}Approve template set</button></div><div className="mt-3 flex flex-wrap gap-3"><button type="button" className="button-outline !border-[#e5a631]/55 !text-[#f4f0e8]" onClick={() => prepareGrowth.mutate()} disabled={prepareGrowth.isPending}>{prepareGrowth.isPending ? <Loader2 size={15} className="animate-spin" /> : <Sparkles size={15} />}Create invite drafts</button><button type="button" className="button-outline !border-[#7c9ee8]/60 !text-[#edf2ff]" onClick={() => prepareEcosystemGrowth.mutate()} disabled={prepareEcosystemGrowth.isPending}>{prepareEcosystemGrowth.isPending ? <Loader2 size={15} className="animate-spin" /> : <Sparkles size={15} />}{prepareEcosystemGrowth.isPending ? "Creating ecosystem drafts" : "Create Gemini + 3S infographic drafts"}</button></div>
      </section>

      <section className="mt-6 rounded-[1.5rem] border border-[#7c9ee8]/45 bg-[linear-gradient(135deg,rgba(124,158,232,.16),rgba(15,23,42,.5))] p-5 shadow-[4px_4px_0_rgba(124,158,232,.13)] lg:p-6" data-testid="ecosystem-infographic-review">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div><p className="font-mono text-[10px] font-bold uppercase tracking-[.14em] text-[#b9cdfb]">Gemini + 3 Serangkai ecosystem education</p><h2 className="mt-3 font-display text-3xl tracking-[-.045em]">Infographic campaign pack · review only</h2><p className="mt-3 max-w-3xl text-sm leading-6 text-[#d7e1dc]">The original ecosystem graphics explain the distinct Gemini Bot EA setup/signal workflow and 3 Serangkai EA macro, basket, and Safe TP workflow. Preparation creates review drafts only. They are not part of the active three-times-daily private-invite queue, and they cannot publish or schedule until you explicitly select each exact item.</p></div><div className="rounded-2xl border border-[#7c9ee8]/45 bg-black/10 px-4 py-3 text-sm text-[#edf2ff]"><p className="font-mono text-[9px] font-bold uppercase tracking-[.12em] text-[#b9cdfb]">Review status</p><p className="mt-1 font-semibold">{counts.ecosystemInfographics} owner-review draft{counts.ecosystemInfographics === 1 ? "" : "s"}</p><p className="mt-1 text-xs leading-5 text-[#c5d5ff]">No new post has been sent or queued.</p></div></div>
      </section>
      ...
      <section className="mt-6 rounded-[1.25rem] border border-[#7db88e]/40 bg-[#7db88e]/10 p-5"><p className="font-mono text-[10px] font-bold uppercase tracking-[.14em] text-[#d3f1d8]">Signal evidence intake · draft only</p><p className="mt-2 max-w-4xl text-sm leading-6 text-[#d7e1dc]">Authenticated Gemini Bot EA setup and TP1/TP2/TP3 screenshots may arrive from the VPS. Each lifecycle event creates an owner-review image draft for optional Telegram-channel marketing, with a portal link and risk notice. It does not auto-publish to Threads, post back to Telegram, or alter the EA signal channel. Review full context and redact account identifiers before selecting any screenshot for publication.</p></section>
