...
export default function Portal() {
  ...
  const claimPurchase = trpc.portal.claimPurchase.useMutation({
    onSuccess: ({ productName, emailDelivery }) => {
      const emailNote = emailDelivery === "sent" ? " A buyer activation email was sent to your signed-in address." : emailDelivery === "already_sent" ? " An activation email was already recorded for this verified claim." : emailDelivery === "not_authorized" ? " Portal access is active; email delivery will begin after the administrator completes Gmail authorization." : emailDelivery === "failed" ? " Portal access is active, but the activation email could not be delivered. Please use the portal guidance below." : "";
      setClaimMessage(`${productName} access is confirmed. Your eligible package files are now available below.${emailNote}`);
      setClaimReceiptNo("");
      library.refetch();
    },
  ...
  return (
    <div className="min-h-screen bg-[#f4f0e8] text-[#17201f]">
      ...
      <main>
        ...
        <section id="installation-guide" className="border-b border-[#17201f]/10 bg-[#fbf9f4] px-5 py-12 lg:px-10">
